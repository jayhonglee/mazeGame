...
public class pathfinder {
	...
	public int[][] bitmap=new int[gm.widh/50][gm.widw/50];
...
//				}
			...
			if(!(0<=temp.x && temp.x< gm.widw/50 && 0<=temp.y && temp.y< gm.widh/50) ||  tempMap[temp.y][temp.x].hl) {
				continue;
			}
			tempMap[temp.y][temp.x]=temp;
			tempMap[temp.y][temp.x].hl=true;
			...
			if(inRange(temp.x+1,temp.y) && !tempMap[temp.y][temp.x+1].hl)
				q.add(new queueEle(temp.x+1,temp.y,temp.c+1+tempMap[temp.y][temp.x+1].cost,Math.abs(temp.x+1-dx)+Math.abs(sy-temp.y)));
			if(inRange(temp.x-1,temp.y) && !tempMap[temp.y][temp.x-1].hl)
				q.add(new queueEle(temp.x-1,temp.y,temp.c+1+tempMap[temp.y][temp.x-1].cost,Math.abs(temp.x-1-dx)+Math.abs(sy-temp.y)));
			if(inRange(temp.x,temp.y+1) && !tempMap[temp.y+1][temp.x].hl)
				q.add(new queueEle(temp.x,temp.y+1,temp.c+1+tempMap[temp.y+1][temp.x].cost,Math.abs(temp.x-dx)+Math.abs(sy-temp.y+1)));
			if(inRange(temp.x,temp.y-1) && !tempMap[temp.y-1][temp.x].hl)
				q.add(new queueEle(temp.x,temp.y-1,temp.c+1+tempMap[temp.y-1][temp.x].cost,Math.abs(temp.x-dx)+Math.abs(sy-temp.y-1)));
		}
		return path;
